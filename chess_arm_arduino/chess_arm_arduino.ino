 ...
 * Wrist Roll (ch 3)                  = DISABLED. Confirmed to be a continuous-
 *                                      rotation / misbehaving unit that spins
 *                                      indefinitely under any signal. Not needed
 *                                      by this project (roll stays neutral the
 *                                      whole game), so it is simply never driven.
 *
...
// ---- Positional joints: wrist pitch=3, wrist roll=4 (disabled), gripper=5 ----
// Physical channel for each (pitch->ch4, roll->ch3, gripper->ch5):
const uint8_t POS_CH[3]  = {4, 3, 5};
int PULSE_MIN[3] = {150, 150, 150};   // wristPitch, wristRoll(unused), gripper
...
void moveTo(const int tgt[]) {
  ...
  // Wrist roll (i == 4) is deliberately skipped — disabled, never driven.
  for (int i = 3; i < N; i++) {
    if (i == 4) continue;
    writePosJoint(i, tgt[i]);
  }
  ...
  delay(150);   // let the positional servos visually settle
}
...
void setup() {
  ...
  // Positional joints: actually home for real. Wrist roll (i == 4) is
  // deliberately kept OFF/unpowered — disabled, never driven, avoids spinning.
  for (int i = 3; i < N; i++) {
    if (i == 4) { pwm.setPWM(POS_CH[i - 3], 0, 4096); continue; }
